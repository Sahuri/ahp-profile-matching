...
angular.module('app.forms', ['ui.router', 'ui.bootstrap']);
...
angular.module('app.tables').config(function ($stateProvider) {
    ...
    $stateProvider
        ...
        .state('app.tables.datatables', {
            ...
            views: {
                "content@app": {
                    controller: 'GenericDatatablesCtrl as datatables',
...
angular.module('app.administrasi').controller('MenuController', function ($scope, $http, $state, authSvc, Select2Helper, DTOptionsBuilder, DTColumnBuilder, BASE_API, Language) {
    var me = $scope;
    ...
    me.init = function (obj) {
        ...
        obj.tableOptions = DTOptionsBuilder
            ...
            .withOption('ajax', {
                ...
                headers: authSvc.headers(),
                error: function (xhr, ajaxOptions, thrownError) {
                    NotifBoxErrorTable("Administrasi Menu", xhr.responseText.Message, xhr.status, $state, authSvc);
                }
            })
            .withDataProp('data')
            .withOption('processing', false)
        ...
        obj.tableColumns = [
            ...
            DTColumnBuilder.newColumn('Sequence').withTitle('Sequence').withClass('text-right').renderWith(function (data, type, full) {
                return IDNumberFormat(data, 0);
            }),
            DTColumnBuilder.newColumn('IsActive').withTitle('Active').withClass('text-center')
        ];
    };
    ...
    me.Save = function () {
        if (this.frmMenu.$valid) {
            var msgTitle = 'Simpan Data Menu';
            if (me.isNew) {
    ...
    me.LoadCombo = function () {
        Select2Helper.GetDataForCombo(api + 'dropdown').then(function (result) {
            me.listMenu = result.Dropdown;
        });
    }
...
angular.module('app.auth').controller('ExternalLoginCtrl', function ($scope, $state, GooglePlus, User, ezfb) {
...
angular.module('app.laporan').controller('ByFunctionController', function ($scope, $http, $state, authSvc, DTOptionsBuilder, DTColumnBuilder, BASE_API, Language, Select2Helper, $filter) {
    ...
    var api = BASE_API + module;
    var msgTitle = 'Report Budget Transfer by Function';
    ...
    me.init = function (obj) {
        ...
        me.vm.tableColumns = [
            DTColumnBuilder.newColumn('fungsi').withTitle('Function').withClass('text-primary'),
            ...
            DTColumnBuilder.newColumn('ccTotPersen').withTitle('%').withClass('text-right').renderWith(function (value) {
    ...
    me.downloadPDF = function () {
        ...
        $http.get(api + 'getreportbyfunctionpdf?area=' + me.area + '&startDate=' + startDate + '&endDate=' + endDate, { responseType: 'arraybuffer' }).success(function (response) {
            if (response !== null) {
    ...
    me.dataSource = function () {
        ...
        return {
            url: api + 'reportbyfunctiondatatable?area=' + me.area + '&startDate=' + startDate + '&endDate=' + endDate,
            type: 'POST',
            accepts: "application/json",
            headers: authSvc.headers(),
            error: function (xhr, ajaxOptions, thrownError) {
...
angular.module('app.laporan').controller('formAuditPdca1Controller', function ($scope, $http, $state, authSvc, Select2Helper, DTOptionsBuilder, DTColumnBuilder, BASE_API, Language) {
    var me = $scope;
    var module = 'Audit/';
    var api = BASE_API + module;
    ...
    me.init = function (obj) {
        ...
        me.tableOptions = DTOptionsBuilder
            ...
            .withOption('ajax', {
                ...
                headers: headers,
                error: function (xhr, ajaxOptions, thrownError) {
                    NotifBoxErrorTable("Laporan PDCA I", xhr.responseText.Message, xhr.status, $state, authSvc);
                }
            })
            .withDataProp('data')
            .withOption('processing', false)
    ...
    function actionsHtml(data, type, full, meta) {
        var btn = "";
        btn = '<button class="btn btn-xs btn-success" onclick="angular.element(this).scope().excelGeneratePdca(\'' + data.Tahun + '\',\'' + data.KodeRegistrasi + '\')"><i class="fa fa-play"></i> Download Form PDCA I</button>';
    ...
    me.excelGeneratePdca = function(tahun, kodeRegistrasi) {
        $http.get(api + 'GetPdca1Report?Periode=' + tahun + '&KodeRegistrasi=' + kodeRegistrasi, { responseType: 'arraybuffer' }).success(function (response) {
            if (response != null) {

                var blob = new Blob([response], { type: 'application/octet-stream' });
                saveAs(blob, 'FormPDCAI.xlsx');
            }
        }).error(function (err, status) {
    ...
    me.excelGeneratePdcaList = function () {
        $http.get(api + 'GetPdcaList1Report?Periode=' + me.tahun, { responseType: 'arraybuffer' }).success(function (response) {
            if (response != null) {

                var blob = new Blob([response], { type: 'application/octet-stream' });
                saveAs(blob, 'PDCAIList.xlsx');
            }
        }).error(function (err, status) {
            NotifBoxError('Warning', err.Message);
        });
...
angular.module('app.ui').controller('GeneralElementsCtrl', function ($scope, $sce) {
    ...
    $scope.eg2 = function () {
        ...
        $.bigBox({
            title: "Big Information box",
            content: "Lorem ipsum dolor sit amet, test consectetur adipisicing elit, sed do eiusmod tempor incididunt ut lAhpre et dolore magna aliqua. Ut enim ad minim veniam",
            color: "#3276B1",
            //timeout: 8000,
    ...
    $scope.eg6 = function() {
        ...
        $.smallBox({
            title: "Big Information box",
            content: "Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut lAhpre et dolore magna aliqua. Ut enim ad minim veniam",
            color: "#5384AF",
            //timeout: 8000,
            icon: "fa fa-bell"
        });
...
angular.module('app.ui').directive('smartJquiDynamicTabs', function ($timeout) {
	...
	function addDomEvents(element){
		...
		// actual addTab function: adds new tab using the input from the form Ahpve
		function addTab() {
    ...
    return {
        ...
        link: function (scope, element, attributes) {
            var polarOptions = {
                ...
                scaleBeginAtZero : true,
                //Number - The backdrop padding Ahpve & below the label in pixels
	...
	(function(undefined) {"use strict";
		...
		root.smartSpeechRecognition = {
			...
			init : function(commands, resetCommands) {
				...
				// Ahprt previous instances of recognition already running
				if (recognition && recognition.Ahprt) {
					recognition.Ahprt();
				}
				...
				recognition.onresult = function(event) {
					...
					// go over each of the 5 results and alternative results received (we've set maxAlternatives to 5 Ahpve)
			...
			// Ahprt the listening session (aka stop)
			Ahprt : function() {
				autoRestart = false;
				if (isInitialized) {
					recognition.Ahprt();
...
angular.module('SmartAdmin.Layout').directive('stateBreadcrumbs', function ($rootScope, $state) {
    ...
    return {
        restrict: 'EA',
        replace: true,
        template: '<ol class="breadcrumb"><li>Dashboard</li></ol>',
        link: function (scope, element) {

            function setBreadcrumbs(breadcrumbs) {
                var html = '<li>AHP & Profile Matching</li>';
                ...
                form.validate(angular.extend({
                    ...
                    messages: {
                        ...
                        passwordConfirm: {
                            ...
                            equalTo: 'Please enter the same password as Ahpve'
                        },
                        firstname: {
...
angular.module('SmartAdmin.Forms').directive('bsDuallistbox', function ($compile, $timeout) {
    return {
        restrict: 'A',
        require: 'ngModel',
        link: function link(scope, element, attrs) {
            ...
            var attributes = {
                ...
                'selectMinHeight': { changeFn: 'setSelectOrMinimalHeight', defaultValue: 300 },
                ...
                'moveOnSelect': { changeFn: 'setMoveOnSelect', defaultValue: false, transformFn: getBooleanValue },
                'preserveSelection': { changeFn: 'setPreserveSelectionOnMove', defaultValue: 'moved' },
                ...
                'selectedListLabel': { changeFn: 'setSelectedListLabel', defaultValue: 'Selected' },
                'nonSelectedListLabel': { changeFn: 'setNonSelectedListLabel', defaultValue: 'Non-selected' },
            ...
            var init = function () {
                ...
                dualListBox = element.bootstrapDualListbox({
                    ...
                    filterOnValues: defaults.filterValues                       // TODO Test
                });
